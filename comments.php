...
<div class="pix-content-wrap">
  <div id="comments">
    <header class="pix-heading-title">
      <h2 class="pix-section-title heading-color"> <?php echo comments_number(__('No Comments', 'Media News'), __('1 Comment', 'Media News'), __('% Comments', 'Media News') );?> </h2>
    </header>
    <ul>
    ...
    <div class="navigation">
      <div class="nav-previous">
        <?php previous_comments_link( __( '<span class="meta-nav">&larr;</span> Older Comments', 'Media News') ); ?>
      </div>
      <div class="nav-next">
        <?php next_comments_link( __( 'Newer Comments <span class="meta-nav">&rarr;</span>', 'Media News') ); ?>
      </div>
    </div>
    <!-- .navigation -->
...
<div class="pix-content-wrap">
  <?php 
			global $post_id;
			$you_may_use = __( 'You may use these <abbr title="HyperText Markup Language">HTML</abbr> tags and attributes: %s', 'Media News');
			$must_login = __( 'You must be <a href="%s">logged in</a> to post a comment.', 'Media News');
			$logged_in_as = __('Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>', 'Media News');
			$required_fields_mark = ' ' . __('Required fields are marked %s', 'Media News');
			...
			$defaults = array( 'fields' => apply_filters( 'comment_form_default_fields', 
				array(
					...
					//'<span>'.( $req ? __( '(required)', 'Media News') : '' ) .'</span>'.
					...
					'title_reply' => __( 'Leave a Comment', 'Media News' ),
					'title_reply_to' => __( 'Leave a Reply to %s', 'Media News' ),
					'cancel_reply_link' => __( 'Cancel reply', 'Media News' ),
					'label_submit' => __( 'Submit', 'Media News' ),); 
